...
static void thread_abs(void *pvParameter)
{
    while (1)
    {
        if (xSemaphoreTake(xMutex, portMAX_DELAY) == pdTRUE) {
            ...
            bool all = gpio_get_level(pinos[ALL]);
            if ((all && aux) == 0)
            {
                usleep(100000);
                estados[BREAK] = true;
...
{   printf("\033[2J\033[H");
    ...
    while (1)
    {
        ...
        if (xSemaphoreTake(xMutex, portMAX_DELAY) == pdTRUE) {
            ...
            {
                printf("\e[1;1H\x1b[32m ACCELERATE\n" );
            }else{
                printf("\e[1;1H\x1b[90m ACCELERATE\n");
            }
            if (estados[BREAK] == true)
            {
                printf("\e[2;1H\x1b[32m BREAK\n");
            }else{
                printf("\e[2;1H\x1b[90m BREAK\n");
            }
            
            if (estados[AIRBAG] == true)
            {
                printf("\e[3;1H\x1b[32m AIRBAG\n");
            }else{
                printf("\e[3;1H\x1b[90m AIRBAG\n");
            }
            if (estados[BELT] == true)
            {
                printf("\e[4;1H\x1b[32m BELT\n");
            }else{
                printf("\e[4;1H\x1b[90m BELT\n");
            }
            if (estados[LOCK] == true)
            {
                printf("\e[5;1H\x1b[32m LOCK\n");
            }else{
                printf("\e[5;1H\x1b[90m LOCK\n");
            }
            if (estados[WINDOW] == true)
            {
                printf("\e[6;1H\x1b[32m WINDOW\n");
            }else{
                printf("\e[6;1H\x1b[90m WINDOW\n");
            }
            if (estados[LIGHTS] == true)
            {
                printf("\e[7;1H\x1b[32m LIGHTS\n");
            }else{
                printf("\e[7;1H\x1b[90m LIGHTS\n");
            }
            if (estados[HOT] == true)
            {
                printf("\e[8;1H\x1b[32m HOT\n");
            }else{
                printf("\e[8;1H\x1b[90m HOT\n");
            }
